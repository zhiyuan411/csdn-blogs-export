 ...
 *      · 左侧栏「分类专栏」面板（#asideCategory → a.special-column-name）列出作者**全部**专栏、
 *        文本带「付费/篇」、无 title 属性，与本文所属专栏无关，是泛化兜底策略的污染源，必须排除。
 *
 * 设计要点：
 *   1. 多策略（SUBJECT_STRATEGIES）按优先级依次尝试，任一命中即返回：
 *      付费专栏卡（服务端直出）→ 头部「收录于」徽标（免费专栏，服务端直出，三种松紧度）
 *      → 头部下拉卡片 → 旧版精确选择器 → 泛化兜底（任意 category_ 链接，排除左侧栏）。
 ...
 * @type {Array<{name: string, type: 'css'|'xpath', selector: string, attr?: string, exclude?: string, description: string}>}
 */
export const SUBJECT_STRATEGIES = [
    ...
    },
    {
        ...
        attr: 'title',
        exclude: '#asideCategory',
        description: '泛化兜底：任意指向专栏页（category_xxx.html）的链接；排除左侧栏「分类专栏」面板（那里列出作者全部专栏、与本文无关且文本带"付费"）'
    }
];
